...
$(document).ready(function(){
	window.addEventListener("message",function(event){
		...
		if (event["data"]["timer"] !== undefined){
			$(".timer").css("height",event["data"]["timer"] );
		}
		...
		if (event["data"]["vehicle"] !== undefined){
			if (event["data"]["vehicle"] == true){
				...
				$(".lataria").html(event["data"]["lataria"]);
			...
			var timerZone = "";
			if (event["data"]["timerZone"] !== undefined){
					timerZone = "<inative>" + event["data"]["timerZone"] + "</inative>"
			}

			$("#Streets").html(event["data"]["street01"] + event["data"]["street02"]);

			$("#displayBoxes").html(lastRadio + teaText + wantedText + userVoice + timerZone  );
		}
