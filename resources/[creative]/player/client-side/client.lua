...
-----------------------------------------------------------------------------------------------------------------------------------------
local Wash = {
	vec3(25.34,-1391.93,29.34),
	vec3(170.59,-1718.43,28.66),
	vec3(850.71,-2110.8,30.03),
...
Citizen.CreateThread(function()
    while true do
        ...
        if not IsEntityInWater(ped) then
            if GetEntityHealth(ped) <= 15 then
                idle = 5
                setHurt()
            elseif hurt and GetEntityHealth(ped) > 16 then
                setNotHurt()
            end
        end
        Citizen.Wait(idle)
...
AddEventHandler("player:checkTrash",function(entity)
	if inTrash then
		local ped = PlayerPedId()
		FreezeEntityPosition(ped,false)
		SetEntityVisible(ped,true,false)
...
Citizen.CreateThread(function()
	local innerTable = {}
	for k,v in pairs(Wash1) do
		local Ped = PlayerPedId()
		table.insert(innerTable,{ v[1],v[2],v[3],5,"E","Car Wash","Pressione para iniciar" })
	end
     ...
     while true do
        ...
        local shot = IsPedShooting(ped)
         if shot == 1 and IsPedInAnyVehicle(ped) then
             ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.03) -- só alterar o valor --
         end
     end
 end)]]
 -------------------------------------- Nao atira falando no radio----------------------
 Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        local ped = PlayerPedId()
