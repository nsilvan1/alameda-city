...
function cRP.requestPossuidos()
	...
	if user_id then
		...
		for k,v in pairs(vehicles) do
			...
			local vehPrices = vehiclePrice(v["vehicle"]) * 0.02
			...
			if vehicleType(v["vehicle"]) == "work" then
				vehPrices = vehiclePrice(v["vehicle"]) * 0.02
			end
	...
	if user_id then
		...
		if vehicle[1] then
			if vehicle[1]["tax"] <= os.time() then
				local vehiclePrice = parseInt(vehiclePrice(vehName) * 0.02)
...
function cRP.requestSell(vehName)
	...
	if user_id then
		if actived[user_id] == nil then
			...
			if vehType == "work" or  vehType == "rental"  then
				TriggerClientEvent("Notify",source,"amarelo","Este veículo não podem ser vendido.",3000)
				actived[user_id] = nil
				return false
			end
