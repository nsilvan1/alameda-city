...
local garageLocates = {
	...
	["62"] = { name = "Police2", payment = false, perm = "Police" },
	["63"] = { name = "heliPolice", payment = false, perm = "Police" },
	["64"] = { name = "Lspd", payment = false, perm = "Police" },
	...
	["160"] = { name = "TaxiAerio", payment = true},
	["161"] = { name = "Trucker", payment = false},
	["162"] = { name = "Entregador", payment = false},
	["163"] = { name = "Caçador", payment = false},
	["164"] = { name = "Jardinagem", payment = false},
	["165"] = { name = "Contrabando", payment = true},
	["166"] = { name = "Delivery", payment = false},
	["167"] = { name = "Delivery", payment = false},
	["168"] = { name = "Minerador", payment = false},
	["169"] = { name = "Kart", payment = false},
	["173"] = { name = "Burgershot", payment = false, perm = "Burgershot" },

	["154"] = { name = "Garage", payment = false, perm = "Mafiavinhedo" },
	["175"] = { name = "Garage", payment = false, perm = "Motoclub" },
	["158"] = { name = "Garage", payment = true }, -- East Custom
	["159"] = { name = "Garage", payment = true },
	["170"] = { name = "Garage", payment = false, perm = "Obey" },
	["176"] = { name = "Garage", payment = false, perm = "Yakuza" },
	["177"] = { name = "Garage", payment = false, perm = "Bunker" },
	["178"] = { name = "Garage", payment = false, perm = "Burgershot" },
	["179"] = { name = "Garage", payment = false, perm = "Ballas" },
	["180"] = { name = "Garage", payment = false }, -- Mansão Dark
	["181"] = { name = "Garage", payment = false }, -- Mansão Camila
	["182"] = { name = "Garage", payment = false } -- Los Santos Customs
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- SIGNALREMOVE
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("garages:Impound",function(vehName)
	...
	if user_id then
		...
		if vRP.request(source,"A liberação do veículo tem o custo de <b>$"..parseFormat(vehPrice * 0.02).."</b> dólares, deseja prosseguir com a liberação do mesmo?","Sim","Não") then
			if vRP.paymentFull(user_id,vehPrice * 0.02) then
				...
				if vehSignal[vehPlate] == nil then
					...
					if os.time() >= parseInt(searchTimers[user_id]) then
						...
						if DoesEntityExist(idNetwork) and not IsPedAPlayer(idNetwork) and GetEntityType(idNetwork) == 2 then
							...
							TriggerClientEvent("",source,"amarelo","Rastreador do veículo foi ativado por <b>30 segundos</b>, lembrando que se o mesmo estiver em movimento a localização pode ser imprecisa.",10000)
						else
							if vehSpawn[vehPlate] then
							...
							TriggerClientEvent("",source,"","A seguradora efetuou o resgate do seu veículo e o mesmo já se encontra disponível para retirada.",5000)
						end
					else
						TriggerClientEvent("",source,"amarelo","Rastreador só pode ser ativado a cada <b>60 segundos</b>.",5000)
					end
				else
					TriggerClientEvent("",source,"amarelo","Rastreador está desativado.",5000)
				end
			else
						...
						if garageLocates[garageName][""] then
							if vRP.userPremium(user_id) then
								...
								if vRP.request(source,"Desejar retirar seu veículo?") then
									if vRP.getBank(user_id) >= parseInt(vehPrice * 0.02) then
										TriggerClientEvent("dynamic:closeSystem",source)
										local netExist,netVeh,mHash = cRP.serverVehicle(vehName,Coords[1],Coords[2],Coords[3],Coords[4],vehPlate,vehicle[1]["nitro"],vehicle[1]["doors"],vehicle[1]["body"])
...
RegisterCommand("dv",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
			if vRP.hasGroup(user_id,"Moderator") or vRP.hasPermission(user_id,"PoliceComando") then
