...
local tableList = {
	...
	["methtable"] = {
		["anim"] = { "anim@amb@business@coc@coc_unpack_cut@","fullcut_cycle_v6_cokecutter" },
		[1] = { ["timer"] = 10, ["need"] = "saline", ["needAmount"] = 2 },
		[2] = { ["timer"] = 10, ["need"] = "acetone", ["needAmount"] = 2 },
		[3] = { ["timer"] = 10, ["item"] = "meth", ["itemAmount"] = 3 }
	},
	["weedtable"] = {
		["anim"] = { "anim@amb@business@coc@coc_unpack_cut@","fullcut_cycle_v6_cokecutter" },
		[1] = { ["timer"] = 10, ["need"] = "silk", ["needAmount"] = 2 },
		[2] = { ["timer"] = 10, ["need"] = "weedleaf", ["needAmount"] = 2 },
...
local lootItens = {
	...
	["Weapons"] = {
		...
		["cooldown"] = 7200,
		["list"] = {
			[1] = { ["item"] = "glass", ["min"] = 2, ["max"] = 5 },
			-- [2] = { ["item"] = "techtrash", ["min"] = 1, ["max"] = 1 },
			[2] = { ["item"] = "polvora", ["min"] = 5, ["max"] = 10 },
			[3] = { ["item"] = "tarp", ["min"] = 2, ["max"] = 5 },
			[4] = { ["item"] = "enxofre", ["min"] = 5, ["max"] = 10 },
			-- [6] = { ["item"] = "sheetmetal", ["min"] = 1, ["max"] = 2 },
			-- [7] = { ["item"] = "explosives", ["min"] = 1, ["max"] = 2 },
			[5] = { ["item"] = "aluminum", ["min"] = 2, ["max"] = 5 },
			[6] = { ["item"] = "copper", ["min"] = 2, ["max"] = 5 },
			[7] = { ["item"] = "plastic", ["min"] = 2, ["max"] = 5 }
		}
	},
	["Supplies"] = {
		["null"] = 75,
...
vRP._prepare(
    "profissao",
    "SELECT permiss, ORGS FROM vrp_permissions WHERE id = @id"
)
-----------------------------------------------------------------------------------------------------------------------------------------
-- REQUESTINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("inventory:Deliver",function(Slot,Amount)
	...
	if user_id then
		...
		if nameItem == "newspaper" then
			if not vRPC.lastVehicle(source,"faggio2") then
				TriggerClientEvent("Notify",source,"amarelo","Precisa utilizar o veículo <b>FAGGIO2</b>.",3000)
				return
			end

			if exports["homes"]:initNewspapers(source) then
				vRP.generateItem(user_id,"dollars",750,true)

				if vRP.userPremium(user_id) then
					vRP.generateItem(user_id,"dollars",75,true)
				end
		...
		if nameItem == "foodbox" then
			...
			if vDELIVER.Deliver(source) then
				if vRP.tryGetInventoryItem(user_id,totalName,Amount,false,Slot) then
					local value = math.random(850,950)
					local value3 = math.random(85,95)
					local value2 = math.random(2,4)
					vDELIVER.Update(source)
					vRP.generateItem(user_id,"dollars",value,true) -- 25 combo a cada 30 min
					vRP.giveInventoryItem(user_id,"coins",value2,true)
					TriggerClientEvent("sounds:source",source,"coin",0.5)
					if vRP.userPremium(user_id) then
						vRP.generateItem(user_id,"dollars",value3,true)
					end
		...
		if itemType(totalName) == "Usável" or itemType(totalName) == "Animal" then
								...
								repeat
									if os.time() >= parseInt(Active[user_id]) then
										...
										vRP.removeInventoryItem(user_id,totalName,1,false)
											...
											vRPC.revivePlayer(n_source,120)
											 TriggerClientEvent("resetBleeding",n_source)
											 TriggerClientEvent("itensNotify",source,{ "removeu",itemIndex(nameItem),parseFormat(1),itemName(nameItem) })
			...
			if nameItem == "identity" then
				if openIdentity[user_id] then
					...
					if identity then
						local orgs = vRP.query("profissao", {id = identity["id"]})
						local profissao = ''
						local cargo = ''
						-- caso não tenha profissão 
						if #orgs == 0 or orgs[1]["ORGS"] == 'Civil'  then 
							profissao = 'Civil'
							cargo = ''
						else 
							profissao = orgs[1]["ORGS"]
							cargo =  orgs[1]["permiss"]
					    end 
						openIdentity[user_id] = true
						if numberIdentity == user_id then
							...
							TriggerClientEvent("vRP:Identity",source,{ mode = "identity", nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], veiculos = identity["garage"], gemas = vRP.userGemstone(identity["steam"]), passaporte = identity["id"], premium = premium, sangue = bloodTypes(identity["blood"]), profissao = profissao, cargo = cargo })
						else
							TriggerClientEvent("vRP:Identity",source,{ mode = "fidentity", nome = identity["name"].." "..identity["name2"], nacionalidade = identity["locate"], passaporte = identity["id"], sangue = bloodTypes(identity["blood"]) , profissao = profissao , cargo = cargo})
						end
					end
				end
			return end
...
--========================================= COMBO POKEMON 3
			...
			if nameItem == "combo3" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 15
				TriggerClientEvent("Progress",source,15000)
				TriggerClientEvent("inventory:Close",source)
				TriggerClientEvent("inventory:Buttons",source,true)
				vRPC.createObjects(source,"missheist_agency2aig_2","look_at_phone_a","ng_proc_food_bag01a",49,60310,-0.05,0.18,0.09,19.0,0.0,130.0)
				...
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							...
							if randomItem <= 1 then                                   ------ SUPREMO
								selectItem = "mewtwo"   
							elseif randomItem >= 2 and randomItem <= 9 then           ------ LENDÁRIO
								selectItem = Pokemon('Lendario')
							elseif randomItem >= 10 and randomItem <= 27 then         ------ ÉPICO
								selectItem = Pokemon('Epico')
							elseif randomItem >= 28 and randomItem <= 49 then         ------ RARO
								selectItem = Pokemon('Raro')
							else
								selectItem =  Pokemon('Comum')
							end

							vRP.generateItem(user_id,"bleeder",1)
							vRP.generateItem(user_id,"fries",1)
							vRP.generateItem(user_id,"orangejuice",1)
				...
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							...
							if randomItem <= 3 then                                   ------ SUPREMO
								selectItem = "mewtwo"                                

							elseif randomItem >= 4 and randomItem <= 15 then           ------ LENDÁRIO
								selectItem = Pokemon('Lendario')
							elseif randomItem >= 16 and randomItem <= 45 then         ------ ÉPICO
								selectItem = Pokemon('Epico')
							elseif randomItem >= 46 and randomItem <= 75 then         ------ RARO
								selectItem =  Pokemon('Raro')
							else
								selectItem =  Pokemon('Comum')
							end

							vRP.generateItem(user_id,"torpedo",1)
							vRP.generateItem(user_id,"shotrings",1)
							vRP.generateItem(user_id,"icecream1",1)
							vRP.generateItem(user_id,"strawberryjuice",1)
							vRP.generateItem(user_id,selectItem,amountItem,true)
					...
					if os.time() >= parseInt(Active[user_id]) then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							...
							if randomItem <= 5 then                                
								selectItem = "mewtwo"                
							elseif randomItem >= 6 and randomItem <= 25 then
								selectItem = Pokemon('Lendario')
							elseif randomItem >= 26 and randomItem <= 67 then       
								selectItem = Pokemon('Epico')
							elseif randomItem >= 68 and randomItem <= 82 then         
								selectItem =  Pokemon('Raro')
							else 
								selectItem =  Pokemon('Comum')
							end 

							vRP.generateItem(user_id,"heartstopper",1)
							vRP.generateItem(user_id,"taco",1)					
							vRP.generateItem(user_id,"milkshake",1)
							vRP.generateItem(user_id,"passionjuice",1)
							vRP.generateItem(user_id,selectItem,amountItem,true)
...
--========================================================================================
			...
			if nameItem == "heartstopper" or nameItem == "moneyshot" or nameItem == "bleeder" or nameItem == "meatfree" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 7
				TriggerClientEvent("Progress",source,7000)
				TriggerClientEvent("inventory:Close",source)
				...
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							if nameItem == "bleeder" or nameItem == "meatfree" then
								vRP.upgradeHunger(user_id,30)
							elseif nameItem == "moneyshot" then
								vRP.upgradeHunger(user_id,50)
							elseif nameItem == "heartstopper" then
			...
			if nameItem == "cannedsoup" or nameItem == "canofbeans" then
				...
				TriggerClientEvent("inventory:Buttons",source,true)
				--vRPC.playAnim(source,true,{"mp_player_inteat@burger","mp_player_int_eat_burger"},true)
				vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","v_ret_247_swtcorn2",49,18905,0.10,0.0,0.08,150.0,320.0,160.0)
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						Active[user_id] = nil
						vRPC.removeObjects(source,"one")
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							if nameItem == "cannedsoup" then
								vRP.upgradeHunger(user_id,35)
							elseif nameItem == "canofbeans" then
								vRP.upgradeHunger(user_id,35)																
							end
						end
					end
			...
			if nameItem == "cannedsoup" or nameItem == "canofbeans" then
				...
				TriggerClientEvent("inventory:Close",source)
				TriggerClientEvent("inventory:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","v_ret_247_swtcorn2",49,60309)
				--vRPC.playAnim(source,true,{"mp_player_inteat@burger","mp_player_int_eat_burger"},true) -----
			...
			if nameItem == "cookedfishfillet" or nameItem == "cookedmeat" then
				...
				TriggerClientEvent("inventory:Buttons",source,true)
				--vRPC.playAnim(source,true,{"mp_player_inteat@burger","mp_player_int_eat_burger"},true)
				vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","v_ret_fh_noodle",49,18905,0.10,0.0,0.08,150.0,320.0,160.0)
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						Active[user_id] = nil
						vRPC.removeObjects(source,"one")
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							if nameItem == "cookedfishfillet" then
								vRP.upgradeHunger(user_id,50)
							elseif nameItem == "cookedmeat" then
								vRP.upgradeHunger(user_id,50)																
							end
						end
					end
			...
			--ABRINDO COMBO =======-- vRPC.createObjects(source,"missheist_agency2aig_2","look_at_phone_a","ng_proc_food_bag01a",49,60310,-0.05,0.18,0.09,19.0,0.0,130.0)
			...
			-- 	TriggerClientEvent("inventory:Buttons",source,true)
			-- 	--vRPC.playAnim(source,true,{"mp_player_inteat@burger","mp_player_int_eat_burger"},true)
			-- 	vRPC.createObjects(source,"amb@code_human_wander_eating_donut@male@idle_a","idle_c","v_ret_fh_noodle",49,28422)
			...
			if nameItem == "icecream1" or nameItem == "icecream2" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 7
				TriggerClientEvent("Progress",source,7000)
				TriggerClientEvent("inventory:Close",source)
				TriggerClientEvent("inventory:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","prop_food_coffee",49,18905,0.10,0.0,0.08,150.0,320.0,160.0)
			...
			if nameItem == "milkshake" then
				...
				TriggerClientEvent("inventory:Close",source)
				TriggerClientEvent("inventory:Buttons",source,true)
				vRPC.createObjects(source,"mp_player_inteat@burger","mp_player_int_eat_burger","prop_food_bs_juice02",49,18905,0.10,0.0,0.08,150.0,320.0,160.0)
			...
			if nameItem == "fries" or  nameItem == "shotrings" or nameItem == "torta" or nameItem == "wrap" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 7
				TriggerClientEvent("Progress",source,7000)
				TriggerClientEvent("inventory:Close",source)
				...
				if otherPlayer then
					if vPLAYER.getHandcuff(otherPlayer) then
						...
						if nuser_id then
							...
							if vRP.hasGroup(nuser_id,"PoliceComando") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									vRP.updatePermission(nuser_id,"PoliceComando","waitPoliceComando")
									...
									TriggerClientEvent("service:Label",otherPlayer,"PoliceComando","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"PoliceSargento") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									vRP.updatePermission(nuser_id,"PoliceSargento","waitPoliceSargento")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"PoliceSargento","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"PoliceSoldado") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									vRP.updatePermission(nuser_id,"PoliceSoldado","waitPoliceSoldado")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"PoliceSoldado","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end

							if vRP.hasGroup(nuser_id,"PoliceRecruta") then
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
									vRP.removePermission(nuser_id,"Police")
									vRP.updatePermission(nuser_id,"PoliceRecruta","waitPoliceRecruta")
									TriggerEvent("blipsystem:serviceExit",otherPlayer)
									TriggerClientEvent("radio:outServers",otherPlayer)
									TriggerClientEvent("vRP:PoliceService",otherPlayer,false)
									TriggerClientEvent("service:Label",otherPlayer,"PoliceRecruta","Entrar em Serviço",5000)
									TriggerClientEvent("Notify",source,"amarelo","Todas as comunicações foram retiradas.",5000)
								end
							end							
...
AddEventHandler("inventory:checkStockade",function(Entity)
	...
	if parseInt(#policeResult) >= 15 then
	...
	if user_id and dismantleList[vehName] and Active[user_id] == nil then
		...
		repeat
			if os.time() >= parseInt(Active[user_id]) then
				...
				vRP.generateItem(user_id,"dollars2",math.random(5500,6000),true)
...
function Pokemon(type)
	...
	if type == 'Raro' then 
		random = math.random(5)    
		if random == 1 then 	
			selectItem = "charmander"
		elseif random == 2 then         
			selectItem = "bulbasaur"
		elseif random == 3 then        
			selectItem = "squirtle"
		elseif random == 4 then        
			selectItem = "eevee"
