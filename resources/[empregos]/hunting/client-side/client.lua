...
local animalHunting = {}
local huntCoords = { -675.33,5829.79,17.32 }
	...
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if distance <= 1.5 then
				...
				if inHunting then
					DrawText3D(huntCoords[1],huntCoords[2],huntCoords[3],"~g~E~w~   FINALIZAR - ~b~CAÇADOR~b~")
				else
					DrawText3D(huntCoords[1],huntCoords[2],huntCoords[3],"~g~E~w~   INICIAR - ~b~CAÇADOR~b~")
				end
				...
				if IsControlJustPressed(1,38) and distance <= 1 then
					...
					if inHunting then
						...
						TriggerEvent("Notify","vermelho","Você saiu do serviço de CAÇADOR",5000)
					else
						inHunting = true
						PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
						TriggerEvent("Notify","verde","Você entrou no serviço de CAÇADOR",5000)
						TriggerEvent("Notify","amarelo","COMPRE na cabana sua ARMA, MUNIÇÃO e CANIVETE para realizar a caça",10000)
			...
			if distance <= 2 then
				if IsPedDeadOrDying(animalHunting[k]) and not IsPedAPlayer(animalHunting[k]) then
					if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_SWITCHBLADE") then
						...
						animalHunting[k] = nil
						Citizen.Wait(1000)
						vRP.playAnim(true,{"anim@gangops@facility@servers@bodysearch@","player_search"},true)   --- -675.33,5829.79,17.32
