			...
			for k,v in pairs(works) do
				...
				if distance <= 2 then
					...
					if not inService then
						nomeServico = v["coords"][4]
						DrawText3D(v["coords"][1],v["coords"][2],v["coords"][3],"~g~E~w~   INICIAR - ~b~"..nomeServico.."~b~")
					else
						DrawText3D(v["coords"][1],v["coords"][2],v["coords"][3],"~g~E~w~   FINALIZAR - ~b~"..nomeServico.."~b~")
					end
					...
					if IsControlJustPressed(1,38) and inSeconds <= 0 then
						...
						if not inService then
							if vSERVER.checkPermission(k) then
								...
								TriggerEvent("Notify","verde","Você entrou no serviço de " ..nomeServico,5000)
							end
						else
							if inService == k then
								lastService = k
								...
								TriggerEvent("Notify","vermelho","Você saiu do serviço de " ..nomeServico ,5000)
								nomeServico = nil
								...
								TriggerEvent("Notify","amarelo","Finalize o emprego de ".. nomeServico .. " para iniciar um novo.",5000)
							end
						end
					end
				end
