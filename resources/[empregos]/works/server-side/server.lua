...
local works = {
	["Transportador"] = {
		["coords"] = { 354.27,271.06,103.04, 'TRANSPORTADOR' },
		["upgradeStress"] = 0.5,
		["routeCollect"] = false,
		["routeDelivery"] = true,
		...
		["deliveryPayment"] = {
			...
			["max"] = 120,
			["item"] = "dollars",
			["item2"] = "coins" -- 38 caixas entregou 96 malotes em 19 min.
		}
	},
	-- ["Lenhador"] = {
	...
	["Mergulhador"] = {
		["coords"] = { 1520.56,3780.08,34.46, 'MERGULHADOR' },
		["upgradeStress"] = 1,
		["routeCollect"] = false,
		["routeDelivery"] = false,
		["usingVehicle"] = false,
	...
	["Colheita"] = {
		["coords"] = { 406.08,6526.16,27.75, 'COLHEITA' },
		["upgradeStress"] = 0.5,
		["routeCollect"] = false,
		["routeDelivery"] = false,
		["usingVehicle"] = false,
	...
	["ColetordeEsterco"] = {
		["coords"] = { 2407.73,4796.0,35.23,'COLETOR DE ESTERCO' },
		["upgradeStress"] = 0.5,
		["routeCollect"] = false,
		["routeDelivery"] = false,
		["usingVehicle"] = false,
		...
		["collectCoords"] = {
			{ 2399.17,4776.28,34.53 },
			{ 2407.53,4767.79,34.29 },
			{ 2415.26,4759.76,34.29 },
			{ 2423.4,4751.19,34.29 },
			{ 2431.46,4743.5,34.29 },
			{ 2439.63,4735.3,34.29 },
			{ 2520.51,4738.38,34.27 },
			{ 2512.41,4747.22,34.31 },
			{ 2504.1,4755.13,34.31 },
			{ 2496.02,4763.9,34.37 },
			{ 2474.3,4762.0,34.31 },
			{ 2465.53,4771.15,34.37 },
			{ 2457.83,4779.18,34.53 },
			{ 2449.65,4787.61,34.66 },
			{ 2441.62,4795.23,34.66 },
			{ 2433.47,4803.66,34.81 }
        },
    ["deliveryItem"] = {
	    "esterco"
        }
	...
	["AgricultorTrigo"] = {
		["coords"] = { 2301.09,5064.78,45.81,'AGRICULTOR' },
		["upgradeStress"] = 0.5,
		["routeCollect"] = false,
		["routeDelivery"] = false,
		["usingVehicle"] = false,
	...
	["Motorista"] = {
		["coords"] = { 453.05,-607.72,28.59, 'MOTORISTA' },
		["upgradeStress"] = 1,
		["routeCollect"] = true,
		["routeDelivery"] = false,
...
-----------------------------------------------------------------------------------------------------------------------------------------
local collectAmount = {}
local collectAmount2 = {}
local paymentAmount = {}
local paymentAmount2 = {}
local deliveryAmount = {}
local deliveryAmount2 = {}
-----------------------------------------------------------------------------------------------------------------------------------------
-- COLLECTCONSUME
-----------------------------------------------------------------------------------------------------------------------------------------
function cRP.collectConsume(serviceName)
	...
	if user_id then
		if works[serviceName]["collectRandom"] then
			...
			if serviceName == "Mergulhador" then
				local randomItem = math.random(100)

				if randomItem <= 1 then
					amountItem = 1
					selectItem = "gemstone"
				elseif randomItem >= 2 and randomItem <= 5 then
					selectItem = "coins"
					amountItem = math.random(50)
				...
				amountItem2 = math.random(works[serviceName]["collectCoins"]["min"],works[serviceName]["collectCoins"]["max"])
			end
			...
			local deliveryItem2 = works[serviceName]["deliveryItem2"]
			collectAmount[user_id] = math.random(works[serviceName]["collectConsume"]["min"],works[serviceName]["collectConsume"]["max"])
			collectAmount2[user_id] = math.random(works[serviceName]["collectCoins"]["min"],works[serviceName]["collectCoins"]["max"])
			...
			if (vRP.inventoryWeight(user_id) + (itemWeight(deliveryItem) * parseInt(collectAmount[user_id]))) <= vRP.getWeight(user_id) then
				...
				vRP.generateItem(user_id,deliveryItem2,collectAmount2[user_id],true)
				TriggerClientEvent("sounds:source",source,"coin",0.5)
...
function cRP.deliveryConsume(serviceName)
	...
	if user_id then
		...
		local deliveryItem2 = works[serviceName]["deliveryPayment"]["item2"]
		...
		paymentAmount2[user_id] = math.random(works[serviceName]["deliveryPayment"]["min2"],works[serviceName]["deliveryPayment"]["max2"])
		...
		if (vRP.inventoryWeight(user_id) + (itemWeight(deliveryItem) * parseInt(paymentAmount[user_id]))) <= vRP.getWeight(user_id) then
			if vRP.tryGetInventoryItem(user_id,works[serviceName]["deliveryItem"],deliveryAmount[user_id]) then
				...
				local paymentPrice2 = parseInt(paymentAmount2[user_id] * deliveryAmount[user_id])

				vRP.generateItem(user_id,deliveryItem,paymentPrice,true)
				vRP.generateItem(user_id,deliveryItem2,paymentPrice2,true)
				TriggerClientEvent("sounds:source",source,"coin",0.5)
