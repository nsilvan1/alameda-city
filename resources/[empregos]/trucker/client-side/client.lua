		...
		if not IsPedInAnyVehicle(ped) then
			...
			if not getPackage then
				...
				if distance <= 3 then
					...
					if initStage then
						...
						if IsControlJustPressed(1,38) and distance <= 1 and inSeconds <= 0 and GetEntityModel(GetPlayersLastVehicle()) == 1518533038 then
						...
						DrawText3D(initCoords[1],initCoords[2],initCoords[3],"~g~E~w~   INICIAR - ~b~TRANSPORTE~b~")

						if IsControlJustPressed(1,38) and distance <= 1 and inSeconds <= 0 then
							deliveryLocates = math.random(#deliveryCoords)
							PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
							TriggerEvent("Notify","verde","Você entrou no serviço de TRANSPORTE",5000)
							TriggerEvent("Notify","amarelo","<b>Obrigatório</b> usar veículo <b>HAULER</b> da garagem a frente, após buzine para receber a carga do trabalho",10000)
							--TriggerEvent("Notify","amarelo","Retire seu hão da garagem e buzine para receber a carga responsável pelo transporte.",7000)
				...
				if distance <= 50 then
					...
					if IsControlJustPressed(1,38) and inSeconds <= 0 and distance <= 5 then
						...
						if modelVehicle == deliveryCoords[deliveryLocates][4] then
							PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
							TriggerEvent("Notify","amarelo","Volte ao ponto de partida e receba o pagamento.",10000)
			...
			if getPackage and packageVehicle == nil then
				...
				if distance <= 50 then
					...
					if GetEntityModel(veh) == 1518533038 then
						timeDistance = 1
