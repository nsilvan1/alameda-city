...
local spawnModels = {
	...
	"ig_denise","ig_devin","a_m_y_dhill_01","ig_dom","a_m_y_downtown_01","ig_dreyfuss","s_m_m_fiboffice_02"
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			for k,v in pairs(initService) do
				...
				if distance <= 2 then
					...
					if serviceStatus then
						DrawText3D(v[1],v[2],v[3],"~g~E~w~ FINALIZAR - ~b~TAXISTA~b~")
					if IsControlJustPressed(1,38) then
						PlaySoundFrontend(-1, "ERROR", "HUD_AMMO_SHOP_SOUNDSET", 1)
						TriggerEvent("Notify","vermelho","Você saiu do serviço de TAXISTA",5000)
					end
					else
						DrawText3D(v[1],v[2],v[3],"~g~E~w~ INICIAR - ~b~TAXISTA~b~")
					if IsControlJustPressed(1,38) then
						PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
						TriggerEvent("Notify","verde","Você entrou no serviço de TAXISTA",5000)
						TriggerEvent("Notify","amarelo","<b>Obrigatório</b> usar veículo <b>TAXI</b> da garagem ao lado para realizar o trabalho",10000)
					end
					end
