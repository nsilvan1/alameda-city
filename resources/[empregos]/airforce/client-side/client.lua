...
local initService = { -1120.41,-2841.3,13.93,147.41 }
-----------------------------------------------------------------------------------------------------------------------------------------
-- STOPVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
local stopVehicle = {
	...
	{ 965.86,42.32,123.12,144.57 },
	{ 910.34,-1681.39,51.12,172.92 },
	{ 2510.72,-342.05,118.18,42.52 },
	{ 1865.17,3647.77,35.6,209.77 },
	{ -475.56,5988.41,31.34,291.97 },
	{ -1877.02,2805.37,32.81,328.82 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- SPAWNPEDS
-----------------------------------------------------------------------------------------------------------------------------------------
local spawnPeds = {
	...
	{ 960.45,26.31,120.22,325.99 }, -- PED CASSINO
	{ 915.82,-1699.71,51.12,175.75 },
	{ 2507.44,-336.83,115.84,320.32 },
	{ 1860.77,3656.7,34.05,212.6 },
	{ -452.2,6005.94,31.83,136.07 },
	{ -1899.89,2815.07,32.81,249.45 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- SPAWNMODELS
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if distance <= 2 then
				...
				if serviceStatus then
					DrawText3D(initService[1],initService[2],initService[3],"~g~E~w~   FINALIZAR - ~b~TÁXI AÉRIO~b~")
					if IsControlJustPressed(1,38) then
						PlaySoundFrontend(-1, "ERROR", "HUD_AMMO_SHOP_SOUNDSET", 1)
						TriggerEvent("Notify","vermelho","Você saiu do serviço de TÁXI AÉRIO",5000)
					end
				else
					DrawText3D(initService[1],initService[2],initService[3],"~g~E~w~   INICIAR - ~b~TÁXI AÉRIO~b~")
					if IsControlJustPressed(1,38) then
						PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
						TriggerEvent("Notify","verde","Você entrou no serviço de TÁXI AÉRIO",5000)
						TriggerEvent("Notify","amarelo","<b>Obrigatório</b> usar helicóptero <b>SUPERVOLITO2</b> da garagem a frente para realizar o trabalho",10000)
					end
				end
					...
					if IsControlJustPressed(1,38) and distance <= 5 then
						if currentStatus then
							...
							if DoesEntityExist(currentPassenger) then
								...
								Citizen.Wait(1000)
								TaskLeaveVehicle(currentPassenger,vehicle,-1671539132)
