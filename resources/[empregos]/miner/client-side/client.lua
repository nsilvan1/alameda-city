...
local coSelected = 0
local timeSeconds = 0
local initService = { 1050.77,-1958.17,31.04 }
-----------------------------------------------------------------------------------------------------------------------------------------
-- POUCHS
-----------------------------------------------------------------------------------------------------------------------------------------
local collect = {
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if distance <= 2 then
				...
				if serviceStatus then
					DrawText3D(initService[1],initService[2],initService[3],"~g~E~w~   FINALIZAR - ~b~MINERADOR~b~")
				else
					DrawText3D(initService[1],initService[2],initService[3],"~g~E~w~   INICIAR - ~b~MINERADOR~b~")
				end
				...
				if IsControlJustPressed(1,38) then
					if serviceStatus then
						...
						TriggerEvent("Notify","vermelho","Você saiu do serviço de MINERADOR",5000)
					else
						currentStatus = false
						serviceStatus = true
						startmineservice()
						...
						TriggerEvent("Notify","verde","Você entrou no serviço de MINERADOR",5000)
					end
				end
			end
		end
