...
local initService = { -840.64,5398.94,34.61 } -- producao 
-- local initService = {-648.01,5468.22,52.47 } -- teste 
-----------------------------------------------------------------------------------------------------------------------------------------
-- DELIVER
-----------------------------------------------------------------------------------------------------------------------------------------
local deliver = {
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if distance <= 1 then
				...
				if serviceStatus then
					DrawText3D(initService[1],initService[2],initService[3],"~g~E~w~   FINALIZAR - ~b~LENHADOR~b~")
				else
					DrawText3D(initService[1],initService[2],initService[3],"~g~E~w~   INICIAR - ~b~LENHADOR~b~")
				end
				...
				if IsControlJustPressed(1,38) then
					if serviceStatus then
						...
						TriggerEvent("Notify","vermelho","Você saiu do serviço de LENHADOR",5000)
						...
						TriggerEvent("Notify","verde","Você entrou no serviço de LENHADOR",5000)
						TriggerEvent("Notify","amarelo","<b>Obrigatório</b> usar veículo <b>RATLOADER</b> da garagem ao lado para realizar o trabalho",10000)
...
-----------------------------------------------------------------------------------------------------------------------------------------
					...
					if collectDis <= 250 then
						...
						if collectDis <= 0.6 and IsControlJustPressed(1,38) and GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_HATCHET") and timeSeconds <= 0 then
							...
							-- vRP.playAnim(true,{"amb@world_human_hammering@male@base","base"},false)
							
							vRP.playAnim(false,{"melee@hatchet@streamed_core","plyr_front_takedown_b"},true)
							vRP.playAnim(true,{"melee@hatchet@streamed_core","plyr_front_takedown_b"},false)						

							TriggerEvent("Progress",1800,"Cortando...")
							Wait(1800)
