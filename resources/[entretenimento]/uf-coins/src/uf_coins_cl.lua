...
RegisterCommand(cfg.commands.openUi, function() 
  if not user_id then 
    ...
    lServer.showNotify('vermelho', "Você não foi autenticado ainda.")
    return false 
  end
...
RegisterNUICallback("playSound", function(data,cb) 
  ...
  elseif data.action == "error" then 
    if data.error then 
      ...
      lServer.showNotify('vermelho',data.erro )
    end
    PlaySound(-1, "Place_Prop_Fail", "DLC_Dmod_Prop_Editor_Sounds", 0, 0, 1)
  elseif data.action == "click" then 
    PlaySoundFrontend(-1, "TENNIS_POINT_WON", "HUD_AWARDS",true)       --------------------------teste
    PlaySound(-1, "Event_Message_Purple","GTAO_FM_Events_Soundset", 0, 0, 1)
...
RegisterNUICallback("rewardReedem", function(data,cb) 
  if(data.index) then 
    ...
    local coins = lServer.getCoinsNew()
