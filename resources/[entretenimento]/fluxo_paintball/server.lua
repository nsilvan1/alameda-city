        ...
        if default.matchInProgress then
            if #default.currentPlayers < Config.RequiredPlayers then -- if there are not enough players online, cancel the current match
                for i = 1, #default.currentPlayers do
                    TriggerClientEvent('fluxo_paintball:stop', default.currentPlayers[i].id)
            ...
            if #default.currentPlayers >= Config.RequiredPlayers then
                ...
                if default.queue.timer - os.time() <= 0 then
                    ...
                    for i = 1, #default.currentPlayers do
                        ...
                        vRP.tryChest(xPlayer,Config.Price)
                        default.prizePool = default.prizePool + Config.Price
                        TriggerClientEvent('fluxo_paintball:start', default.currentPlayers[i].id)
                    end
                end
            end
        end
    ...
    while true do
        ...
        if not default.matchInProgress then
            ...
            if default.matchEnds - os.time() <= Config.DisplayWinner then
                ...
                vRP.delBank(xPlayer,default.prizePool-4000)
                ...
                for i = 1, #default.currentPlayers do
                    TriggerClientEvent('fluxo_paintball:matchOver', default.currentPlayers[i].id, json.encode(winner), json.encode(default.currentPlayers[i]))
                end
                default = json.decode(json.encode(defaultDefault))
        ...
        TriggerClientEvent('fluxo_paintball:queueInfo', -1, text, other)
        Wait(500)
    end
end)

RegisterServerEvent('fluxo_paintball:join')
AddEventHandler('fluxo_paintball:join', function()
    local src = source
    local xPlayer = vRP.getUserId(source)
    if vRP.getBank(xPlayer) >= Config.Price then
        if not default.matchInProgress then
            local init = false
            for i = 1, #default.currentPlayers do
                if default.currentPlayers[i] then
        ...
        TriggerClientEvent('fluxo_paintball:hudNotify', src, Config.Translations['no_money'])
    end
end)

RegisterServerEvent('fluxo_paintball:kill')
AddEventHandler('fluxo_paintball:kill', function(killed)
    local src = source
    if default.matchInProgress then
        ...
        if isplayerActive(killed) then
            for i = 1, #default.currentPlayers do
                if default.currentPlayers[i].id == killed then
                    ...
                    TriggerClientEvent('fluxo_paintball:die', default.currentPlayers[i].id)
                end
            end
        end
        if killedPlaying then
            for i = 1, #default.currentPlayers do
                if default.currentPlayers[i].id == src then
                    ...
                    TriggerClientEvent('fluxo_paintball:hudNotify', default.currentPlayers[i].id, Config.Translations['you_killed'] .. ': ~r~' .. GetPlayerName(killed))
                elseif default.currentPlayers[i].id == killed then
                    TriggerClientEvent('fluxo_paintball:hudNotify', default.currentPlayers[i].id, Config.Translations['you_got_killed'] .. ': ~r~' .. GetPlayerName(src))
                    TriggerClientEvent('fluxo_paintball:died', default.currentPlayers[i].id, src)
                else
                    TriggerClientEvent('fluxo_paintball:hudNotify', default.currentPlayers[i].id, GetPlayerName(killed) .. Config.Translations['killed_by'] .. GetPlayerName(src))
                end
            end
        end
    end
