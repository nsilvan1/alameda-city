...
CreateThread(function()
  vRP._prepare('uf/addCar','INSERT INTO summerz_vehicles(user_id,vehicle,tax) VALUES(@user_id,@vehicle,@tax)')
...
function lServer.reedemGift(data)
  ...
  if user_id then
    ...
    print('name - '.. giftTable.name)
    print('idname - '..  giftTable.idname)
    print('lastHour - '..tonumber(data.lastHour))
    print('randAmount - ' .. randAmount)
    -- lInfo[user_id].lastGift = {name = giftTable.name, idname = giftTable.idname, amount = randAmount, lastHour = tonumber(data.lastHour)} --- erro
    ...
    if giftTable.idname == "money" then 
      vRP.giveBankMoney(user_id,randAmount)
...
function sendMessage(msgData)
  if string.match(msgData.message,"nome") or string.match(msgData.message,"sobrenome") then 
    ...
    local identity = vRP.query("characters/getUsers",{ id = msgData.user_id })
    name  = identity[1]["name"]
    name2 = identity[1]["name2"]
    if name then 
