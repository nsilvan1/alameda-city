...
CreateThread(function() 
  ...
  if cfg.actions.login.chat_notify.enable then 
    TriggerEvent('chat:addMessage', {
      template = '<div style="display:flex;align-items:center;justify-content:center;color:#ffffff;font-weight: bolder; font-family: Arial, Helvetica, sans-serif;padding:10px;margin:5px 0;background-image: linear-gradient(to right, rgba(1, 35, 50,1) 3%, rgba(0, 0, 0,0) 95%);border-radius: 8px;">{0}</div>',
      args = { cfg.actions.login.chat_notify.message }
    })
    -- lServer.showNotify('amarelo',cfg.actions.login.chat_notify.message)
  end
  if cfg.actions.login.event_notify.enable then 
    -- TriggerEvent('Notify','sucesso',cfg.actions.login.event_notify.message)
    lServer.showNotify('amarelo',cfg.actions.login.event_notify.message)
  end
...
function lClient.init()
  ...
  lServer.showNotify('verde', cfg.actions.login.event_notify.message)
end

openNui = function() 
  if GetEntityHealth(PlayerPedId()) <= 101 then print('sem vida') return end
  if not userInfo then 
    -- TriggerEvent("Notify", "negado","Evento não inicializado.") 
    lServer.showNotify('vermelho', 'Evento não inicializado.')
    return end
