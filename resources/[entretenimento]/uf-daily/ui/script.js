...
const getRandomGift   = () =>{
  ...
  $.post(`https://${resName}/reedemGift`,JSON.stringify({probability: probability/10,  itemIndex: randomGift+1, lastHour: nearMeta}),function(data){
    ...
    if(data.success){
      ...
      setupLastItem(data.item.name,data.item.amount)
      setTimeout(() => {
        $("#baloes").hide()
...
window.addEventListener('message', function (ev) {
  switch (ev.data.case) {
    case 'setup':
      ...
      if(ev.data.lastItem){
        setupLastItem(ev.data.lastItem.name,ev.data.lastItem.amount)  
      } else {
        $('#itemRecebido').hide()
...
const initialFunction = (hours,lastItem)=>{
    ...
    $('#barProguess span').css("width",`${4.24*hours}%`); 
    // -- mudei aqui 4.16*hours
    ...
    $(".nextcheck").text(`${nearMeta} ${(typeof nearMeta == 'number') ? ' prêmio por dia. 1x cada 4 horas.' : ''}`)
