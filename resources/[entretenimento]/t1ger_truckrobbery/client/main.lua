...
local DzSV = Tunnel.getInterface("t1ger_truckrobbery") 
...
Citizen.CreateThread(function()
    while true do
		...
		if distance < cfg.computer.draw.dist then
			sleep = false
			DrawText3D(cfg.computer.pos[1], cfg.computer.pos[2], cfg.computer.pos[3], cfg.computer.draw.text)
				...
				if distance < 2.0 then
					...
					if cops >= cfg.police.minCops then
							...
							if cooldown == nil then
								if DzSV.verifyItens(cfg.computer.item,cfg.computer.dropitem ) then 
									local hasMoney = DzSV.getJobFees()
									if hasMoney then
										OpenHackFunction()
									else
										DzSV.notiFyPerson(Lang['not_enough_money'] )
									end
								end 
							else
								DzSV.notiFyPerson(Lang['cooldown_time_left']:format(cooldown) )
							end
					   
					else
						DzSV.notiFyError(Lang['not_enough_police'] )
					end
				else
					DzSV.notiFyPerson('...')
				end
...
function HackCallback(success)
	...
	if success then
		...
		DzSV.notiFyError(Lang['hacking_failed'])
	end
end
...
AddEventHandler('t1ger_truckrobbery:startJobCL',function()
    ...
    if takenNum == 100 then
		...
		DzSV.notiFyPerson(Lang['no_available_jobs'])
    else
        TriggerEvent('t1ger_truckrobbery:truckRobberyJob', num)
        PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		DzSV.notiFyPerson(Lang['start_time_left'])
    end
end)
		...
		if job.inUse then 
			...
			if distance < 150.0 then 
				...
				if distance < 100.0 and not ArmoredTruckSpawned then
					...
					ArmoredTruck = CreateArmoredTruck('riot', job.pos)
				...
				if ArmoredTruckSpawned and SecuritySpawned then 
					...
					if truck_dist <= 5.0 and not TruckDemolished then
						...
						if GetEntityModel(closest_veh) == GetHashKey('riot') then
							...
							if distVeh < 2.0 then
								DrawText3D(veh_pos.x, veh_pos.y, veh_pos.z, Lang['open_truck_door'])
								local cfg = Config.TruckRobbery
								if IsControlJustPressed(1, 47) and DzSV.verifyItensExplose(cfg.computer.itemExplose ) then 
						...
						if truck_dist < 4.5 then
							...
							if GetEntityModel(closest_veh) == GetHashKey('riot') then
								...
								if truck_dist < 2.0 then
									DrawText3D(veh_pos.x, veh_pos.y, veh_pos.z, Lang['rob_the_truck'])
...
function BlowTheTruckDoor()
	...
	if IsVehicleStopped(ArmoredTruck) then
		...
		DzSV.notiFyPerson(Lang['begin_to_rob'])
		-- TriggerServerEvent('t1ger_truckrobbery:PoliceNotifySV')
	else
		DzSV.notiFyPerson(Lang['truck_not_stopped'])
	end
end
...
function RobbingTheMoney()
	local cfg = Config.TruckRobbery
	TriggerEvent("Progress",(cfg.rob.takeLootTimer * 2500),"Pegando...")
	...
	-- aqui aumenta o tempo da animação para pegar o dinheiro 
	
	Citizen.Wait((cfg.rob.takeLootTimer * 2500))
