...
function cRP.startExercice(qtd)
	...
	-- 	TriggerClientEvent("Notify",source,"acad","Adquirido "..qtd.."KG na mochila",8000)
	-- end
end
...
function cRP.stopExercice()
	...
	if user_id then
		-- TriggerClientEvent("Notify",source,"amarelo","Sessão de exercícios finalizada!",5000)
 ...
 function cRP.checkBackpack()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.getWeight(user_id) == 121 then
